    ...
    def __init__(self, mode="Pathfinder", map=os.path.dirname(__file__) + "/resources/drone_3d/map_1.npy",
        ...
        if mode == "Pathfinder":
            try:
                ...
                # path = json.load(f)
                # self.tracking_path=[p["position"]for p in path ]
                # self.tracking_path = np.array([p["position"] for p in path])
            except IOError:
                print("No map file.")
    ...
    class Viewer:
        def __init__(self, env):
            ...
            if self.env.tracking_path:
                ...
                path_indices = np.ravel([[i, i + 1] for i in range(int(self.tracking_path.size / 3 - 1))])
                self.path_indices = np.array(path_indices, dtype=np.uint32)
                print(self.path_indices)
                print(self.path_indices.size)

                self.EBO = glGenBuffers(1)
                glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, self.EBO)
                glBufferData(GL_ELEMENT_ARRAY_BUFFER, self.path_indices.nbytes, self.path_indices, GL_STATIC_DRAW)
            ...
            self.is_tracking = glGetUniformLocation(shader, "is_tracking")
        ...
        def render(self):
            if self.env.mode == "Pathfinder":
                ...
                while 1:
                    ...
                    # glUniform1i(self.is_tracking, False)
                    ...
                    # glDrawElements(GL_LINES, len(self.path_indices), GL_UNSIGNED_INT, None)

                    glUniform1i(self.is_tracking, True)
                    glBindVertexArray(self.VAO[0])
